...
var Id = new Uint8Array(new Array(16).fill(0).map((_, i2) => i2));
...
var RIPEMD160 = class extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let group = 0; group < 5; group++) {
      ...
      for (let i2 = 0; i2 < 16; i2++) {
        const tr3 = rotl(ar2 + f(rGroup, br2, cr2, dr3) + R_BUF[rr3[i2]] + hbr, sr2[i2]) + er3 | 0;
...
function Zi(t, e = 24) {
  ...
  for (let r2 = 24 - e; r2 < 24; r2++) {
    ...
    for (let s = 0; s < 10; s += 2) {
      const c = (s + 8) % 10, a = (s + 2) % 10, u = n2[a], l2 = n2[a + 1], f5 = rr(u, l2, 1) ^ n2[c], h3 = or(u, l2, 1) ^ n2[c + 1];
      for (let y3 = 0; y3 < 50; y3 += 10) t[s + y3] ^= f5, t[s + y3 + 1] ^= h3;
    }
    let o2 = t[2], i2 = t[3];
    for (let s = 0; s < 24; s++) {
...
var Es = (t, e) => {
  function n2(r2, ...o2) {
    ...
    return { encrypt(l2, f5) {
      ...
      return a = true, tt(l2), c(s.encrypt.length, f5), s.encrypt(l2, f5);
    }, decrypt(l2, f5) {
      if (tt(l2), i2 && l2.length < i2) throw new Error("invalid ciphertext length: smaller than tagLength=" + i2);
      return c(s.decrypt.length, f5), s.decrypt(l2, f5);
    } };
  }
...
function Is(t, e, n2, r2, o2, i2, s, c) {
  const a = o2.length, u = new Uint8Array(ge), l2 = mt(u), f5 = Ye(o2) && Ye(i2), h3 = f5 ? mt(o2) : Tr, y3 = f5 ? mt(i2) : Tr;
  for (let E2 = 0; E2 < a; s++) {
...
function Ns(t, e) {
  ...
  return Fe(o2), Fe(s), Ar(i2), Ar(n2), (c, a, u, l2, f5 = 0) => {
    tt(c), tt(a), tt(u);
    const h3 = u.length;
    if (l2 === void 0 && (l2 = new Uint8Array(h3)), tt(l2), Fe(f5), f5 < 0 || f5 >= Ur) throw new Error("arx: counter overflow");
    ...
    const O2 = mt(a);
    return Is(t, d2, v, O2, u, l2, f5, s), jt(...y3), l2;
  };
}
...
var Us = class {
  ...
  process(e, n2, r2 = false) {
    const o2 = r2 ? 0 : 2048, { h: i2, r: s } = this, c = s[0], a = s[1], u = s[2], l2 = s[3], f5 = s[4], h3 = s[5], y3 = s[6], E2 = s[7], p = s[8], d2 = s[9], v = F(e, n2 + 0), m2 = F(e, n2 + 2), O2 = F(e, n2 + 4), N = F(e, n2 + 6), $ = F(e, n2 + 8), B3 = F(e, n2 + 10), A2 = F(e, n2 + 12), T = F(e, n2 + 14);
    ...
    x = C4 >>> 13, C4 &= 8191, C4 += g * (5 * h3) + w * (5 * f5) + b * (5 * l2) + I2 * (5 * u) + R3 * (5 * a), x += C4 >>> 13, C4 &= 8191;
    let P3 = x + S2 * a + L * c + U4 * (5 * d2) + _ * (5 * p) + j * (5 * E2);
    x = P3 >>> 13, P3 &= 8191, P3 += g * (5 * y3) + w * (5 * h3) + b * (5 * f5) + I2 * (5 * l2) + R3 * (5 * u), x += P3 >>> 13, P3 &= 8191;
    let k2 = x + S2 * u + L * a + U4 * c + _ * (5 * d2) + j * (5 * p);
    x = k2 >>> 13, k2 &= 8191, k2 += g * (5 * E2) + w * (5 * y3) + b * (5 * h3) + I2 * (5 * f5) + R3 * (5 * l2), x += k2 >>> 13, k2 &= 8191;
    let M3 = x + S2 * l2 + L * u + U4 * a + _ * c + j * (5 * d2);
    x = M3 >>> 13, M3 &= 8191, M3 += g * (5 * p) + w * (5 * E2) + b * (5 * y3) + I2 * (5 * h3) + R3 * (5 * f5), x += M3 >>> 13, M3 &= 8191;
    let D = x + S2 * f5 + L * l2 + U4 * u + _ * a + j * c;
    x = D >>> 13, D &= 8191, D += g * (5 * d2) + w * (5 * p) + b * (5 * E2) + I2 * (5 * y3) + R3 * (5 * h3), x += D >>> 13, D &= 8191;
    let z = x + S2 * h3 + L * f5 + U4 * l2 + _ * u + j * a;
    x = z >>> 13, z &= 8191, z += g * c + w * (5 * d2) + b * (5 * p) + I2 * (5 * E2) + R3 * (5 * y3), x += z >>> 13, z &= 8191;
    let Z2 = x + S2 * y3 + L * h3 + U4 * f5 + _ * l2 + j * u;
    x = Z2 >>> 13, Z2 &= 8191, Z2 += g * a + w * c + b * (5 * d2) + I2 * (5 * p) + R3 * (5 * E2), x += Z2 >>> 13, Z2 &= 8191;
    let st2 = x + S2 * E2 + L * y3 + U4 * h3 + _ * f5 + j * l2;
    x = st2 >>> 13, st2 &= 8191, st2 += g * u + w * a + b * c + I2 * (5 * d2) + R3 * (5 * p), x += st2 >>> 13, st2 &= 8191;
    let W = x + S2 * p + L * E2 + U4 * y3 + _ * h3 + j * f5;
    x = W >>> 13, W &= 8191, W += g * l2 + w * u + b * a + I2 * c + R3 * (5 * d2), x += W >>> 13, W &= 8191;
    let J2 = x + S2 * d2 + L * p + U4 * E2 + _ * y3 + j * h3;
    x = J2 >>> 13, J2 &= 8191, J2 += g * f5 + w * l2 + b * u + I2 * a + R3 * c, x += J2 >>> 13, J2 &= 8191, x = (x << 2) + x | 0, x = x + C4 | 0, C4 = x & 8191, x = x >>> 13, P3 += x, i2[0] = C4, i2[1] = P3, i2[2] = k2, i2[3] = M3, i2[4] = D, i2[5] = z, i2[6] = Z2, i2[7] = st2, i2[8] = W, i2[9] = J2;
  }
  finalize() {
...
function _s(t, e, n2, r2, o2, i2 = 20) {
  let s = t[0], c = t[1], a = t[2], u = t[3], l2 = e[0], f5 = e[1], h3 = e[2], y3 = e[3], E2 = e[4], p = e[5], d2 = e[6], v = e[7], m2 = o2, O2 = n2[0], N = n2[1], $ = n2[2], B3 = s, A2 = c, T = a, S2 = u, L = l2, U4 = f5, _ = h3, j = y3, g = E2, w = p, b = d2, I2 = v, R3 = m2, x = O2, C4 = N, P3 = $;
  ...
  let k2 = 0;
  r2[k2++] = s + B3 | 0, r2[k2++] = c + A2 | 0, r2[k2++] = a + T | 0, r2[k2++] = u + S2 | 0, r2[k2++] = l2 + L | 0, r2[k2++] = f5 + U4 | 0, r2[k2++] = h3 + _ | 0, r2[k2++] = y3 + j | 0, r2[k2++] = E2 + g | 0, r2[k2++] = p + w | 0, r2[k2++] = d2 + b | 0, r2[k2++] = v + I2 | 0, r2[k2++] = m2 + R3 | 0, r2[k2++] = O2 + x | 0, r2[k2++] = N + C4 | 0, r2[k2++] = $ + P3 | 0;
}
...
var qs = class extends ke {
  ...
  digestInto(e) {
    ...
    for (let f5 = 0; f5 < u; f5++) c.setUint32(4 * f5, l2[f5], i2);
  }
  digest() {
    const { buffer: e, outputLen: n2 } = this;
    this.digestInto(e);
...
var Fs = class extends qs {
  ...
  process(e, n2) {
    ...
    for (let f5 = 16; f5 < 64; f5++) {
      const h3 = bt[f5 - 15], y3 = bt[f5 - 2], E2 = ct(h3, 7) ^ ct(h3, 18) ^ h3 >>> 3, p = ct(y3, 17) ^ ct(y3, 19) ^ y3 >>> 10;
      bt[f5] = p + bt[f5 - 7] + E2 + bt[f5 - 16] | 0;
    }
    let { A: r2, B: o2, C: i2, D: s, E: c, F: a, G: u, H: l2 } = this;
    for (let f5 = 0; f5 < 64; f5++) {
      const h3 = ct(c, 6) ^ ct(c, 11) ^ ct(c, 25), y3 = l2 + h3 + Ds(c, a, u) + Ks[f5] + bt[f5] | 0, p = (ct(r2, 2) ^ ct(r2, 13) ^ ct(r2, 22)) + Hs(r2, o2, i2) | 0;
...
function Vr(t, e, n2) {
  ...
  }, c = (...f5) => n2(o2, r2, ...f5), a = (f5 = Qe()) => {
    o2 = c(kr([0]), f5), r2 = c(), f5.length !== 0 && (o2 = c(kr([1]), f5), r2 = c());
  }, u = () => {
...
function dc(t, e) {
  return { constTimeNegate: rn, hasPrecomputes(n2) {
    ...
    const u = BigInt(2 ** n2 - 1), l2 = 2 ** n2, f5 = BigInt(n2);
    for (let h3 = 0; h3 < i2; h3++) {
      const y3 = h3 * s;
      let E2 = Number(o2 & u);
      o2 >>= f5, E2 > s && (E2 -= l2, o2 += ve);
...
function hc(t, e, n2, r2) {
  ...
  let l2 = o2;
  for (let f5 = u; f5 >= 0; f5 -= s) {
    a.fill(o2);
    for (let y3 = 0; y3 < r2.length; y3++) {
      const E2 = r2[y3], p = Number(E2 >> BigInt(f5) & BigInt(c));
      a[p] = a[p].add(n2[y3]);
    }
    let h3 = o2;
    for (let y3 = a.length - 1, E2 = o2; y3 > 0; y3--) E2 = E2.add(a[y3]), h3 = h3.add(E2);
    if (l2 = l2.add(h3), f5 !== 0) for (let y3 = 0; y3 < s; y3++) l2 = l2.double();
  }
  return l2;
}
function Wr(t) {
...
BigInt(8);
function bc(t) {
  const e = BigInt(10), n2 = BigInt(20), r2 = BigInt(40), o2 = BigInt(80), i2 = un, c = t * t % i2 * t % i2, a = it(c, Xr, i2) * c % i2, u = it(a, yc, i2) * t % i2, l2 = it(u, wc, i2) * u % i2, f5 = it(l2, e, i2) * l2 % i2, h3 = it(f5, n2, i2) * f5 % i2, y3 = it(h3, r2, i2) * h3 % i2, E2 = it(y3, o2, i2) * y3 % i2, p = it(E2, o2, i2) * y3 % i2, d2 = it(p, e, i2) * l2 % i2;
...
function Ic(t) {
  ...
  const { ProjectivePoint: a, normPrivateKeyToScalar: u, weierstrassEquation: l2, isWithinCurveOrder: f5 } = Ac({ ...e, toBytes(g, w, b) {
...
//# sourceMappingURL=chunk-C6TCCADW.js.map
